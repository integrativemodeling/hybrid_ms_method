...
c     
c     parameter "len" changed to xxx (where xxx is another number) throughout:
c     use "sed s/len=15000/len=xxx/"
c
c     ***************************************************************
c
c
...
c     len is probably the number of atoms
      parameter (len=15000)
...
c     commented out mobil2 to save time--db(12/14/07) 
...
c
      ...
      parameter (len=15000)
      dimension imass(len),xmass(len),xrad(len)
      common/printswitch/ip,it,iu1,iu2,iu3,iv,im2,im4,igs
      common/constants/mu,ro,eo,pi,cang,ro2,dipol,emax,m1,m2,
     ?xe,xk,xn,mconst,correct,romax,inatom,icoord,iic
      common/charge/pcharge(len)
...
c      imass(iatom)=nint(ximass)
      xrad(iatom)=real(ximass)
      if(unit.eq.'au') then
...
c             
c     anyatom (with radius a floating point decimal value)
c
c     ignored in PA calculation: xmass, eolj, rolj
c     rhs is only important value
c
c     based on hydrogen atom
c      if(imass(iatom).eq.1) then 
c      itest=1 
      xmass(iatom)=1.008d0 
      eolj(iatom)=0.65d-03*xe
      rolj(iatom)=2.38d0*1.0d-10  
      rhs(iatom)=xrad(iatom)*1.0d-10
c      rhs(iatom)=2.2d0*1.0d-10
c      endif 
...
C              Default Initialization by Multiplicative Congruential
      IF (NOTYET) THEN
         ...
         KOUNT = 0
         MKOUNT = 0
         WRITE(8,'(A,I2,A,I4)')  ' RANLUX DEFAULT LUXURY LEVEL =  ',
     +        LUXLEV,'      p =',LP
            TWOM24 = 1.
         DO 25 I= 1, 24
            TWOM24 = TWOM24 * 0.5
  ...
  200 CONTINUE
      ...
      LUXLEV = ISD
        IF (LUXLEV .LE. MAXLEV) THEN
          NSKIP = NDSKIP(LUXLEV)
          WRITE (8,'(A,I2)') ' RANLUX LUXURY LEVEL SET BY RLUXIN TO: ',
     +                         LUXLEV
        ELSE  IF (LUXLEV .GE. 24) THEN
          NSKIP = LUXLEV - 24
          WRITE (8,'(A,I5)') ' RANLUX P-VALUE SET BY RLUXIN TO:',LUXLEV
        ELSE
          NSKIP = NDSKIP(MAXLEV)
          WRITE (8,'(A,I5)') ' RANLUX ILLEGAL LUXURY RLUXIN: ',LUXLEV
          LUXLEV = MAXLEV
        ENDIF
      INSEED = -1
      RETURN
...
C                    Entry to initialize from one or three integers
  ...
  310       CONTINUE
      ...
      IF (LUXLEV .LE. MAXLEV)  THEN
         NSKIP = NDSKIP(LUXLEV)
         WRITE(8,'(A,I2,A,I4)') ' RANLUX LUXURY LEVEL SET BY RLUXGO :',
     +        LUXLEV,'     P=', NSKIP+24
      ELSE
          NSKIP = LUXLEV - 24
          WRITE (8,'(A,I5)') ' RANLUX P-VALUE SET BY RLUXGO TO:',LUXLEV
      ENDIF
      IN24 = 0
...
c     ***************************************************************
c
      subroutine testxrand
      implicit double precision (a-h,o-z)
      dimension istore(1000)
      common/xrandom/i1,i2,i3,i4,i5,i6
c
      itti=10000000
      write(8,600) itti
  600 format(/1x,'testing XRAND',
     ?//1x,'printing out repeats of the first number sampled in',
     ?i12,' samples',/)
      do 1001 i=1,1000
 1001 istore(i)=0
      hold=xrand()
      do 1000 i=1,itti
      hold3=xrand()
      if(hold3.eq.hold) write(8,*) i,hold,hold3
      k=dint(hold3*1000.d0)
      istore(k+1)=istore(k+1)+1
 1000 continue
      write(8,610)
  610 format(/1x,'Samples binned into 1000 bins',/)
      do 2000 i=1,1000
 2000 write(8,*) i,istore(i)
c
      return
      end
c
